...
impl Database {
    pub fn new() -> Result<Self> {
        ...
        let _ = conn.execute("ALTER TABLE agents ADD COLUMN active_task TEXT", []);
    ...
    pub fn save_agent_state(&self, state: &AgentState) -> Result<()> {
        ...
        self.conn.execute(
            "INSERT OR REPLACE INTO agents (name, personality, memory, last_seen, mood, xp, active_task) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
            params![
                &state.name,
                &state.personality,
                memory_json,
    ...
    pub fn get_agent_state(&self, name: &str) -> Result<Option<AgentState>> {
        let mut stmt = self.conn.prepare("SELECT personality, memory, last_seen, mood, xp, active_task FROM agents WHERE name = ?1")?;
        
        if let Some(row_result) = stmt.query_map([name], |row| {
            Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?, row.get(5)?))
        })?.next() {
            let (personality, memory_json, last_seen_str, mood, xp, active_task): (String, String, String, String, u32, Option<String>) = row_result?;
